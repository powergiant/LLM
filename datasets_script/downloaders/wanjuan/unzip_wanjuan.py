...
if __name__ == "__main__":
    ...
    with open(project_dir + "/cache_folder.txt", "r") as file:
        cache_dir = file.read()
    # cache_dir = os.path.join(project_dir, "data_downloaded")
    download_dir = os.path.join(cache_dir, "step0_rawdata/wanjuan")
    if not os.path.exists(download_dir):
        os.mkdir(download_dir)
    dir_list = glob.glob(download_dir + "/**/*.jsonl.tar.gz", recursive=True)
    target_root_dir = os.path.join(cache_dir, "step2_data_cleaning/wanjuan")
    print(dir_list)
    print(len(dir_list))
